...
public class EgovFileTool {
	...
	public static ArrayList<String> getExistDirectory(String baseDirPath, String targetDirPath, int cnt) throws Exception {
	...
	public static boolean checkFileExstBySize(String dir, long sizeFrom, long sizeTo) throws Exception {
		...
		if (srcDrctry.exists() && srcDrctry.isDirectory()) {
			...
			File[] fileArray = srcDrctry.listFiles();
			ArrayList<String> list = getSubFilesBySize(fileArray, sizeFrom, sizeTo);
			if (list != null && list.size() > 0) {
				result = true;
			}
		}
					...
					try {
						...
						if (b_err != null)
							try {
								...
								LOGGER.error("{}", e); // 2012.10.15 보안점검 후속조치
							}
					}
				}
				//프로세스 실행 성공시 결과 확인
	 ...
	 * @exception Exception
	 */
	public static ArrayList<String> getFileListByUpdtPd(String drctry, String updtFrom, String updtTo) throws Exception {
	...
	public static boolean copyFilesByOwner(String source, String owner, String target) throws Exception {
		...
		if (srcFile.exists() && srcFile.isDirectory()) {
			...
			File[] fileArray = srcFile.listFiles();
			ArrayList<String> list = getSubFilesByOwner(fileArray, owner);
